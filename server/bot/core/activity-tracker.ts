...
export class ActivityTracker extends EventEmitter {
	...
	start() {
		if (this.isRunning)
			return
		this.isRunning = true
		botLogger.info('[Activity Tracker] Starting chatter activity loop...')
	...
	stop() {
		...
		botLogger.info('[Activity Tracker] Stopped activity loop.')
	}
	...
	async tick() {
		try {
			...
			botLogger.error({ err }, '[Activity Tracker] Error running activity tick')
		}
	}
}
