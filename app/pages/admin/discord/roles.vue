...
import { ConfigAccordion, ConfigAccordionGroup } from '~/components/ui/config-accordion'
import {
	SettingsGroup,
	SettingsGroupAction,
			...
			<div v-else class="flex w-full max-w-3xl flex-col gap-6">
				...
				<Alert v-if="!form.isDiscordConnected" variant="warning" class="border-amber-500/50 bg-amber-500/10">
					...
					>
						...
						You can still modify and save bestowing preferences, but automatic role assignment is inactive and switches are locked.
				...
				<ConfigAccordionGroup constrained>
					<ConfigAccordion
						...
						title="Discord Role Bestowing"
						description="Automatically assign configured roles to new users when they join your Discord server."
					>
						<template #icon>
