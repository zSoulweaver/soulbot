...
export const AlertsSettingsSchema = z.object({
	...
	eventsubAlertFollow: z.string().max(500, 'Follow alert template is too long').default('Thank you for the follow, $(sender)!'),
	eventsubAlertSub: z.string().max(500, 'Subscription alert template is too long').default('Thank you for subscribing, $(sender)! Welcome to the club!'),
	eventsubAlertGift: z.string().max(500, 'Sub-gift alert template is too long').default('Thank you @$(sender) for gifting $(giftCount) sub(s) to the community!'),
	eventsubAlertCheer: z.string().max(500, 'Cheer alert template is too long').default('Thank you @$(sender) for cheering $(bitsCount) bits! $(cheerMessage)'),
	eventsubAlertRaid: z.string().max(500, 'Raid alert template is too long').default('Thank you for the raid, $(sender) with $(raidSize) viewers!'),
	eventsubAlertLive: z.string().max(500, 'Live alert template is too long').default('We are now live playing $(liveGame) - $(liveTitle)!'),
	eventsubAlertOffline: z.string().max(500, 'Offline alert template is too long').default('Stream has ended. Thanks for hanging out!'),
	eventsubAlertBan: z.string().max(500, 'Ban alert template is too long').default('$(sender) has been banned from the channel.'),
	eventsubAlertTimeout: z.string().max(500, 'Timeout alert template is too long').default('$(sender) has been timed out for $(duration) seconds.'),
	eventsubAlertUnban: z.string().max(500, 'Unban alert template is too long').default('$(sender) has been unbanned.'),
	eventsubAlertMessageDelete: z.string().max(500, 'Message delete alert template is too long').default('A message from $(sender) was deleted.'),
